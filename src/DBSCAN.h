...
* REAMME *
* @author: Nezha
* @describe: This is a DBSCAN clustering method implement
*/
...
public:
    ...
    * @param: disfunc {DistanceFunc} : Distance function recall. Euclidian distance is recommanded, but you can replace it by any metric measurement function
...
template<typename T, typename Float>
int Crystal::DBSCAN<T, Float>::Run(
    TVector*                V
    , const uint            dim
    , const Float           eps
    , const uint            min
...
template<typename T, typename Float>
void Crystal::DBSCAN<T, Float>::destroyKdtree() {
    kd_free(this->_kdtree);
}

template<typename T, typename Float>
void Crystal::DBSCAN<T, Float>::buildKdtree(const TVector* V)
{
